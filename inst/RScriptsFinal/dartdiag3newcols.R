...
######## Network x closness
####################################################
#"fig" and "dataReady" folders need to be created before running this code, to save data and figures
...
setwd("E:/WD Apps for Windows/Mar2017BUEXTFull/NEW/TAMANI/SOCSCI/NVIVO/Nodes/Final/")

######### KC: Importing data
nodes <- read.csv("Nodes_dartdiag.csv", header=T, as.is=T)
links <- read.csv("Relationships_dartdiag.csv", header=T, as.is=T)
...
#I used i-graph which uses its weight as a resistance to movement, therefore weights are transformed
links$weight<- 1 - links$weight/(max(links$weight)+min(links$weight)/max(links$weight))

net <- graph.data.frame(links, nodes, directed=T)
saveRDS(net, "network.Rds")
# V(net)
...
pal <- c(lighten("#edac35",30),"#00ac89",darken("#64c4eb",10))[as.factor(nodes$Type)]
pal2 <- sapply(pal, darken, 30)
#HW small code change for colour coding (Type -> type_from)
links.pal <- c(lighten("#edac35",30),"#00ac89",darken("#64c4eb",10) )[as.factor(links$type_from)]




######### KC: Here the plot start
png("fig/diag4.png", width=10, height=10, units="in", res=300)
  ...
  ## KC: NODES
  points(nodes$x, nodes$y, cex=0.2*nodes$No.sources, pch=21, col = pal, bg=pal, lwd=1.5)
...
sqs <- c(5,10,30)
sql <- c(1,5,10)
sqt <- c("Information", "Process", "Combined")
legend(-.8, 1, legend = paste0(sqs, " sources"), pt.cex=0.2*sqs, pch=21, y.intersp=2, x.intersp=2, bty="n")
sqs <- c(5,10,30)
legend(-.3, 1, legend = paste0(sql, " connections"), lwd = .75*sql, y.intersp=2, x.intersp=2, bty="n")
legend(.3,1, legend = paste0(sqt), pt.cex=5, pch=19, col=c( "#00ac89",darken("#64c4eb",10),lighten("#edac35",30) ),
bg=c( "#00ac89",darken("#64c4eb",10),lighten("#edac35",30 )), y.intersp=2, x.intersp=2, bty="n")
